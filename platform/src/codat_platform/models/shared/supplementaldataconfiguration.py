...
class SupplementalDataConfiguration(BaseModel):
    ...
    """ # noqa: E501
    supplemental_data_config: Optional[Dict[str, SupplementalDataSourceConfiguration]] = Field(default=None, alias="supplementalDataConfig")
    __properties: ClassVar[List[str]] = ["supplementalDataConfig"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of SupplementalDataConfiguration from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each value in supplemental_data_config (dict)
        _field_dict = {}
        if self.supplemental_data_config:
            for _key_supplemental_data_config in self.supplemental_data_config:
                if self.supplemental_data_config[_key_supplemental_data_config]:
                    _field_dict[_key_supplemental_data_config] = self.supplemental_data_config[_key_supplemental_data_config].to_dict()
            _dict['supplementalDataConfig'] = _field_dict
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of SupplementalDataConfiguration from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "supplementalDataConfig": dict(
                (_k, SupplementalDataSourceConfiguration.from_dict(_v))
                for _k, _v in obj["supplementalDataConfig"].items()
            )
            if obj.get("supplementalDataConfig") is not None
            else None
        })
        return _obj
...
class SupplementalDataConfigurationTypedDict(TypedDict):
    """TypedDict variant of SupplementalDataConfiguration, for dict-literal request/response inputs."""
    supplemental_data_config: NotRequired[Optional[Dict[str, SupplementalDataSourceConfigurationTypedDict]]]
