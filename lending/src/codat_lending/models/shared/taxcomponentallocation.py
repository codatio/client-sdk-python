...
class TaxComponentAllocation(BaseModel):
    ...
    """ # noqa: E501
    tax_component_ref: Optional[TaxComponentRef] = Field(default=None, alias="taxComponentRef")
    rate: Annotated[OptionalNullable[Decimal], BeforeValidator(validate_decimal), PlainSerializer(serialize_decimal(False))] = Field(default=UNSET, description="Tax amount on order line sale as available from source commerce software.")
    __properties: ClassVar[List[str]] = ["taxComponentRef", "rate"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of TaxComponentAllocation from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of tax_component_ref
        if self.tax_component_ref:
            _dict['taxComponentRef'] = self.tax_component_ref.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of TaxComponentAllocation from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "taxComponentRef": TaxComponentRef.from_dict(obj["taxComponentRef"]) if obj.get("taxComponentRef") is not None else None,
            "rate": obj.get("rate")
        })
        return _obj
...
class TaxComponentAllocationTypedDict(TypedDict):
    """TypedDict variant of TaxComponentAllocation, for dict-literal request/response inputs."""
    tax_component_ref: NotRequired[Optional[TaxComponentRefTypedDict]]
