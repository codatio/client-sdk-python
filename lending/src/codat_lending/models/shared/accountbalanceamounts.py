...
class AccountBalanceAmounts(BaseModel):
    ...
    """ # noqa: E501
    available: Annotated[OptionalNullable[Decimal], BeforeValidator(validate_decimal), PlainSerializer(serialize_decimal(False))] = Field(default=UNSET, description="The balance available in the account, including any pending transactions. This doesn't include additional funds available from any overdrafts.")
    current: Annotated[Optional[Decimal], BeforeValidator(validate_decimal), PlainSerializer(serialize_decimal(False))] = Field(default=None, description="The balance of the account only including cleared transactions.")
    limit: Annotated[OptionalNullable[Decimal], BeforeValidator(validate_decimal), PlainSerializer(serialize_decimal(False))] = Field(default=UNSET, description="The minimum allowed balance for the account. For example, a $100.00 overdraft would show as a limit of `-100.00`.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["available", "current", "limit"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of AccountBalanceAmounts from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of AccountBalanceAmounts from a dict"""
        if obj is None:
            return None
...
class AccountBalanceAmountsTypedDict(TypedDict):
    """TypedDict variant of AccountBalanceAmounts, for dict-literal request/response inputs."""
    available: NotRequired[Optional[Decimal]]
    current: NotRequired[Optional[Decimal]]
    limit: NotRequired[Optional[Decimal]]
