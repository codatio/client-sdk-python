...
class BankingTransaction(BaseModel):
    ...
    def _serialize_drop_none(self, handler):
        ...
        _nullable = {'description', 'merchantName', 'merchant_name'}
        return {
    ...
    id: str = Field(description="The unique identifier of the bank transaction.")
    account_id: str = Field(description="The unique identifier of the bank account.", alias="accountId")
    description: OptionalNullable[str] = Field(default=UNSET, description="The description of the bank transaction.")
    amount: Annotated[Optional[Decimal], BeforeValidator(validate_decimal), PlainSerializer(serialize_decimal(False))] = Field(default=None, description="The amount of the bank transaction.")
    currency: str = Field(description="The currency of the bank transaction.")
    posted_date: Optional[str] = Field(default=None, description="In Codat's data model, dates and times are represented using the <a class=\"external\" href=\"https://en.wikipedia.org/wiki/ISO_8601\" target=\"_blank\">ISO 8601 standard</a>. Date and time fields are formatted as strings; for example:  ``` 2020-10-08T22:40:50Z 2021-01-01T00:00:00 ```    When syncing data that contains `DateTime` fields from Codat, make sure you support the following cases when reading time information:  - Coordinated Universal Time (UTC): `2021-11-15T06:00:00Z` - Unqualified local time: `2021-11-15T01:00:00` - UTC time offsets: `2021-11-15T01:00:00-05:00`  > Time zones >  > Not all dates from Codat will contain information about time zones.   > Where it is not available from the underlying platform, Codat will return these as times local to the business whose data has been synced.", alias="postedDate")
    authorized_date: Optional[str] = Field(default=None, description="In Codat's data model, dates and times are represented using the <a class=\"external\" href=\"https://en.wikipedia.org/wiki/ISO_8601\" target=\"_blank\">ISO 8601 standard</a>. Date and time fields are formatted as strings; for example:  ``` 2020-10-08T22:40:50Z 2021-01-01T00:00:00 ```    When syncing data that contains `DateTime` fields from Codat, make sure you support the following cases when reading time information:  - Coordinated Universal Time (UTC): `2021-11-15T06:00:00Z` - Unqualified local time: `2021-11-15T01:00:00` - UTC time offsets: `2021-11-15T01:00:00-05:00`  > Time zones >  > Not all dates from Codat will contain information about time zones.   > Where it is not available from the underlying platform, Codat will return these as times local to the business whose data has been synced.", alias="authorizedDate")
    code: Optional[TransactionCode] = None
    merchant_name: OptionalNullable[str] = Field(default=UNSET, description="The name of the merchant.", alias="merchantName")
    transaction_category_ref: Optional[TransactionCategoryRef] = Field(default=None, alias="transactionCategoryRef")
    __properties: ClassVar[List[str]] = ["modifiedDate", "sourceModifiedDate", "id", "accountId", "description", "amount", "currency", "postedDate", "authorizedDate", "code", "merchantName", "transactionCategoryRef"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of transaction_category_ref
        if self.transaction_category_ref:
            _dict['transactionCategoryRef'] = self.transaction_category_ref.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "transactionCategoryRef": TransactionCategoryRef.from_dict(obj["transactionCategoryRef"]) if obj.get("transactionCategoryRef") is not None else None
        })
        return _obj
...
class BankingTransactionTypedDict(TypedDict):
    """TypedDict variant of BankingTransaction, for dict-literal request/response inputs."""
    ...
    posted_date: NotRequired[Optional[str]]
    authorized_date: NotRequired[Optional[str]]
    code: NotRequired[Optional[TransactionCode]]
    merchant_name: NotRequired[Optional[str]]
    transaction_category_ref: NotRequired[Optional[TransactionCategoryRefTypedDict]]
