...
class AccountPrototype(BaseModel):
    ...
    r"""Type of account"""
    valid_datatype_links: Annotated[OptionalNullable[List[ValidDataTypeLinks]], pydantic.Field(alias="validDatatypeLinks")] = UNSET
    r"""The validDatatypeLinks can be used to determine whether an account can be correctly mapped to another object; for example, accounts with a `type` of `income` might only support being used on an Invoice and Direct Income. For more information, see [Valid Data Type Links](/lending-api#/schemas/ValidDataTypeLinks)."""
    
    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = ["currency", "currentBalance", "description", "fullyQualifiedCategory", "fullyQualifiedName", "isBankAccount", "name", "nominalCode", "status", "supplementalData", "type", "validDatatypeLinks"]
