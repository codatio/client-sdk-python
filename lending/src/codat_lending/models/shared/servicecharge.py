...
class ServiceCharge(BaseModel):
    ...
    ServiceCharge
    """ # noqa: E501
    description: Optional[str] = Field(default=None, description="Service charges for this order.")
    total_amount: Annotated[Optional[Decimal], BeforeValidator(validate_decimal), PlainSerializer(serialize_decimal(False))] = Field(default=None, description="Total amount of the service charge, including tax.", alias="totalAmount")
    tax_percentage: Annotated[Optional[Decimal], BeforeValidator(validate_decimal), PlainSerializer(serialize_decimal(False))] = Field(default=None, description="Percentage rate (from 0 to 100) of any tax applied to the service charge.", alias="taxPercentage")
    tax_amount: Annotated[Optional[Decimal], BeforeValidator(validate_decimal), PlainSerializer(serialize_decimal(False))] = Field(default=None, description="Amount of the service charge that is tax.", alias="taxAmount")
    taxes: Optional[List[TaxComponentAllocation]] = Field(default=None, description="Taxes breakdown as applied to service charges.")
    quantity: Optional[int] = Field(default=None, description="The number of times the charge is charged.")
    type: Optional[ServiceChargeType] = None
    __properties: ClassVar[List[str]] = ["description", "totalAmount", "taxPercentage", "taxAmount", "taxes", "quantity", "type"]
...
class ServiceChargeTypedDict(TypedDict):
    """TypedDict variant of ServiceCharge, for dict-literal request/response inputs."""
    ...
    type: NotRequired[Optional[ServiceChargeType]]
