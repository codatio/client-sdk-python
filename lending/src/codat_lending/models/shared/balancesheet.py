...
class BalanceSheet(BaseModel):
    ...
    assets: Optional[ReportLine] = Field(default=None, description="ReportLines for assets. For example, fixed and current assets.")
    liabilities: Optional[ReportLine] = Field(default=None, description="ReportLines for liabilities. For example, current liabilities.")
    net_assets: Annotated[Decimal, BeforeValidator(validate_decimal), PlainSerializer(serialize_decimal(False))] = Field(description="Value of net assets for a company in their base currency.", alias="netAssets")
    equity: Optional[ReportLine] = Field(default=None, description="ReportLines for equities. For example, retained and current year earnings. See below.")
    __properties: ClassVar[List[str]] = ["date", "assets", "liabilities", "netAssets", "equity"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "date": obj.get("date"),
            "assets": ReportLine.from_dict(obj["assets"]) if obj.get("assets") is not None else None,
            "liabilities": ReportLine.from_dict(obj["liabilities"]) if obj.get("liabilities") is not None else None,
            "netAssets": obj.get("netAssets"),
            "equity": ReportLine.from_dict(obj["equity"]) if obj.get("equity") is not None else None
        })
        return _obj
...
class BalanceSheetTypedDict(TypedDict):
    ...
    assets: NotRequired[Optional[ReportLineTypedDict]]
    liabilities: NotRequired[Optional[ReportLineTypedDict]]
    net_assets: NotRequired[Decimal]
    equity: NotRequired[Optional[ReportLineTypedDict]]
