...
class AccountingAttachment(BaseModel):
    ...
    content_type: Annotated[OptionalNullable[str], pydantic.Field(alias="contentType")] = UNSET
    r"""File type of the attachment. This is represented by appending the file type to the [IETF standard file naming requirements](https://tools.ietf.org/html/rfc6838). For example, for a jpeg file the output is **image/jpeg**.
    ...
    r"""Identifier for the attachment, unique for the company in the accounting software."""
    include_when_sent: Annotated[Optional[bool], pydantic.Field(alias="includeWhenSent")] = None
    r"""If `true`, then the attachment is included with the associated invoice, bill or direct costs when it is printed, emailed, or sent to a customer, if the underlying accounting software allows this."""
    ...
    def serialize_model(self, handler):
        optional_fields = ["contentType", "dateCreated", "fileSize", "id", "includeWhenSent", "modifiedDate", "name", "sourceModifiedDate"]
        nullable_fields = ["contentType", "fileSize", "name"]
        null_default_fields = []
