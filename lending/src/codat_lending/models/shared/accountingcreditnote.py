...
class AccountingCreditNote(BaseModel):
    ...
    def _serialize_drop_none(self, handler):
        ...
        _nullable = {'creditNoteNumber', 'credit_note_number', 'currencyRate', 'currency_rate', 'lineItems', 'line_items', 'note', 'paymentAllocations', 'payment_allocations', 'withholdingTax', 'withholding_tax'}
        return {
    ...
    id: Optional[str] = Field(default=None, description="Identifier for the credit note, unique to the company in the accounting software.")
    credit_note_number: OptionalNullable[str] = Field(default=UNSET, description="Friendly reference for the credit note.", alias="creditNoteNumber")
    customer_ref: Optional[AccountingCustomerRef] = Field(default=None, description="Reference to the customer the credit note has been issued to.", alias="customerRef")
    withholding_tax: OptionalNullable[List[Items]] = Field(default=UNSET, alias="withholdingTax")
    total_amount: Annotated[Decimal, BeforeValidator(validate_decimal), PlainSerializer(serialize_decimal(False))] = Field(description="Total amount of credit that has been applied to the customer's accounts receivable", alias="totalAmount")
    total_discount: Annotated[Decimal, BeforeValidator(validate_decimal), PlainSerializer(serialize_decimal(False))] = Field(description="Any discounts applied to the credit note amount.", alias="totalDiscount")
    sub_total: Annotated[Decimal, BeforeValidator(validate_decimal), PlainSerializer(serialize_decimal(False))] = Field(description="Value of the credit note, including discounts and excluding tax.", alias="subTotal")
    additional_tax_amount: Annotated[Optional[Decimal], BeforeValidator(validate_decimal), PlainSerializer(serialize_decimal(False))] = Field(default=None, description="Additional tax amount applied to credit note.", alias="additionalTaxAmount")
    additional_tax_percentage: Annotated[Optional[Decimal], BeforeValidator(validate_decimal), PlainSerializer(serialize_decimal(False))] = Field(default=None, description="Percentage rate of any additional tax applied to the credit note.", alias="additionalTaxPercentage")
    total_tax_amount: Annotated[Decimal, BeforeValidator(validate_decimal), PlainSerializer(serialize_decimal(False))] = Field(description="Any tax applied to the credit note amount.", alias="totalTaxAmount")
    discount_percentage: Annotated[Decimal, BeforeValidator(validate_decimal), PlainSerializer(serialize_decimal(False))] = Field(description="Percentage rate (from 0 to 100) of discounts applied to the credit note.", alias="discountPercentage")
    remaining_credit: Annotated[Decimal, BeforeValidator(validate_decimal), PlainSerializer(serialize_decimal(False))] = Field(description="Unused balance of totalAmount originally raised.", alias="remainingCredit")
    status: CreditNoteStatus = Field(description="Current state of the credit note.")
    ...
    line_items: OptionalNullable[List[CreditNoteLineItem]] = Field(default=UNSET, alias="lineItems")
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of AccountingCreditNote from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of AccountingCreditNote from a dict"""
        if obj is None:
            return None
...
class AccountingCreditNoteTypedDict(TypedDict):
    """TypedDict variant of AccountingCreditNote, for dict-literal request/response inputs."""
    modified_date: NotRequired[Optional[str]]
    source_modified_date: NotRequired[Optional[str]]
    id: NotRequired[Optional[str]]
    credit_note_number: NotRequired[Optional[str]]
    ...
    line_items: NotRequired[Optional[List[CreditNoteLineItemTypedDict]]]
